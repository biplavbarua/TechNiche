...
import { analyzeIdea, AnalysisResult } from "../lib/api"; // Fixed import path
import { ArrowRight, Scale, AlertTriangle, CheckCircle, Search, FileText } from "lucide-react";
      ...
      <main className="pt-32 pb-20 px-6 max-w-5xl mx-auto">
        <div className="text-center mb-16 space-y-4">
          <motion.div
            ...
            className="inline-flex items-center px-3 py-1 rounded-full border border-indigo-500/30 bg-indigo-500/10 text-indigo-300 text-xs font-medium mb-4"
          >
        ...
        <motion.div
          ...
          className="bg-slate-900/50 border border-indigo-500/20 rounded-2xl p-2 shadow-2xl backdrop-blur-sm"
        >
          <div className="relative">
            <textarea
              ...
              className="w-full h-40 bg-slate-950/50 rounded-xl p-6 text-lg text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 resize-none transition-all"
            />
            <div className="absolute bottom-4 right-4">
              <button
                onClick={handleAnalyze}
                disabled={loading || !idea.trim()}
                className="flex items-center gap-2 px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-indigo-500/20 active:scale-95"
              >
                {loading ? (
            ...
            >
              ...
              <div className="space-y-6">
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl">
                  ...
                  <ul className="space-y-3">
                    {result.cited_cases.map((caseName, index) => (
                      <li key={index} className="flex items-start gap-3 p-3 rounded-lg bg-slate-950/50 hover:bg-slate-800 transition-colors cursor-pointer border border-slate-800 hover:border-indigo-500/30 group">
                        <div className="mt-1 w-2 h-2 rounded-full bg-indigo-500 group-hover:bg-indigo-400 transition-colors" />
                        <span className="text-sm text-slate-400 group-hover:text-slate-200 transition-colors">{caseName}</span>
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6">
                  <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-2">Disclaimer</h3>
